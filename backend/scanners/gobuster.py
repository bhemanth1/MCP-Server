...
def safe_run(args, timeout=300, cwd=None):
    try:
        ...
        if not executable:
            ...
            if not tool_path:
                ...
                if os.path.exists(exe_path):
                    ...
                    return -1, "", f"Tool '{tool_name}' not found in PATH. Install: go install github.com/OJ/gobuster/v3@latest. Then add {go_bin} to PATH."
            else:
        ...
        go_bin = os.path.join(os.environ.get("USERPROFILE", ""), "go", "bin")
        return -1, "", f"Tool not found: {str(e)}. Install: go install github.com/OJ/gobuster/v3@latest. Add {go_bin} to PATH and restart."
...
def run_and_parse(target: str, raw_dir: str) -> dict:
    ...
    try:
        from scanners.enhanced_gobuster import run_and_parse as enhanced_parse
        ...
        import os
        sys.path.insert(0, os.path.dirname(__file__))
        from enhanced_gobuster import run_and_parse as enhanced_parse
        return enhanced_parse(target, raw_dir)